...
exports.up = function(knex) {
    const filePath = path.join(__dirname, '..', 'mssp_triggers.sql');
    const sql = fs.readFileSync(filePath, 'utf8');
  
    // Split the SQL file content by custom delimiters (e.g., $$) and remove empty lines/statements
    ...
    return statements.reduce((promise, statement) => {
      return promise.then(() => knex.raw(statement));
    }, Promise.resolve());
  };
  ...
  exports.down = function(knex) {
    ...
    return knex.raw(getTriggerNamesSQL)
      .then((result) => {
        ...
        return Promise.all(dropTriggerPromises);
      });
  };
...
//  */
// exports.down = function(knex) {
//     return knex.schema.dropTable('logtable');
// };
